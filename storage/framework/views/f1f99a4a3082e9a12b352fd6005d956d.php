...
<?php $__env->startSection('breadcrumb', 'Admin › Data Guru'); ?>
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
            <a href="<?php echo e(route('admin.teachers.export')); ?>" class="inline-flex items-center gap-2 rounded-xl bg-green-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-green-700 transition">
                Export Excel
            </a>
            ...
            <a href="<?php echo e(route('admin.teachers.create')); ?>" class="inline-flex items-center gap-2 rounded-xl bg-navy px-4 py-2 text-sm font-semibold text-white shadow hover:bg-opacity-90 transition">
    ...
    <form method="GET" class="rounded-2xl bg-white p-4 shadow-sm">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-end">
            ...
            <!-- Reset Link -->
            <?php if(request()->hasAny(['search', 'subject'])): ?>
                <a href="<?php echo e(route('admin.teachers.index')); ?>" class="rounded-lg border border-slate-200 px-6 py-2.5 font-medium text-slate-700 transition hover:bg-slate-50">
                    Reset
                </a>
            <?php endif; ?>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $teachers ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $teacher): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="<?php echo e($i % 2 === 1 ? 'bg-slate-50' : ''); ?> border-t border-slate-200 transition hover:bg-slate-100">
                            ...
                            <td class="px-6 py-4 text-sm font-medium text-slate-900">
                                <?php echo e(($teachers->currentPage() - 1) * $teachers->perPage() + $i + 1); ?>
                            ...
                            <td class="px-6 py-4 text-sm">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-medium text-slate-900"><?php echo e($teacher->user->name ?? '-'); ?></p>
                                        <p class="text-xs text-slate-500"><?php echo e($teacher->user->email ?? '-'); ?></p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm text-slate-900">
                                <?php echo e($teacher->subject?->name ?? '-'); ?>
                            ...
                            <td class="px-6 py-4 text-sm text-slate-900">
                                <span class="inline-block rounded-full bg-purple-100 px-3 py-1 text-xs font-medium text-purple-700">
                                    <?php echo e($teacher->classes?->count() ?? 0); ?> kelas
                                </span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <!-- Edit Button -->
                                    <a href="<?php echo e(route('admin.teachers.edit', $teacher)); ?>" class="rounded-lg p-2 text-slate-600 transition hover:bg-slate-100 hover:text-gold" title="Edit">
                                        <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    ...
                                    >
                                        <div class="rounded-2xl bg-white p-6 shadow-2xl max-w-sm mx-4">
                                            ...
                                            <p class="mb-6 text-center text-sm text-slate-600">Data guru <strong><?php echo e($teacher->user->name ?? 'ini'); ?></strong> akan dihapus permanen.</p>
                                            <div class="flex gap-3">
                                                ...
                                                <form method="POST" action="<?php echo e(route('admin.teachers.destroy', $teacher)); ?>" class="flex-1">
                                                    <?php echo csrf_field(); ?>
    ...
    <!-- Pagination -->
    <?php if($teachers && $teachers->total() > 0): ?>
        <div class="flex items-center justify-between">
            <p class="text-sm text-slate-600">
                Menampilkan <strong><?php echo e(($teachers->currentPage() - 1) * $teachers->perPage() + 1); ?></strong>–<strong><?php echo e(min($teachers->currentPage() * $teachers->perPage(), $teachers->total())); ?></strong> dari <strong><?php echo e($teachers->total()); ?></strong> guru
            </p>
            <div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\sistem-akademik-smpn13\resources\views/admin/teachers.blade.php ENDPATH**/ ?>