...
<div class="space-y-6">
            ...
            <form method="POST" action="{{ route('admin.announcements.store') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        class="w-full rounded-xl border-2 border-slate-200 px-4 py-2.5 text-sm transition focus:border-navy focus:outline-none focus:ring-2 focus:ring-navy/20 @error('title') border-red-500 @enderror"
                        required
                    >
                    @error('title')
                ...
                <div>
                    ...
                    <textarea
                        ...
                        class="w-full rounded-xl border-2 border-slate-200 px-4 py-2.5 text-sm transition focus:border-navy focus:outline-none focus:ring-2 focus:ring-navy/20 @error('content') border-red-500 @enderror"
                        required
                ...
                <div>
                    ...
                    <input
                        ...
                        class="w-full rounded-xl border-2 border-slate-200 px-4 py-2.5 text-sm transition focus:border-navy focus:outline-none focus:ring-2 focus:ring-navy/20 @error('published_at') border-red-500 @enderror"
                        required
                    >
                    @error('published_at')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
            ...
            >
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <p class="text-xs text-slate-500">
                        Dipublikasikan: {{ $announcement->published_at ? $announcement->published_at->format('d M Y H:i') : '-' }}
                    </p>
                </div>
                ...
                <div class="flex flex-shrink-0 gap-2">
                    <!-- Edit Button -->
                    <a href="{{ route('admin.announcements.edit', $announcement) }}" class="rounded-lg p-2 text-slate-600 transition hover:bg-slate-200 hover:text-gold" title="Edit">
                        <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    ...
                    >
                        <div class="rounded-2xl bg-white p-6 shadow-2xl max-w-sm mx-4">
                            ...
                            <p class="mb-6 text-center text-sm text-slate-600">Pengumuman <strong>{{ $announcement->title }}</strong> akan dihapus permanen.</p>
                            <div class="flex gap-3">
                                ...
                                <form method="POST" action="{{ route('admin.announcements.destroy', $announcement) }}" class="flex-1">
                                    @csrf
                                    @method('DELETE')
    ...
    @if($announcements && method_exists($announcements, 'links'))
        <div class="flex items-center justify-between pt-4">
            <p class="text-sm text-slate-600">
                Menampilkan <strong>{{ ($announcements->currentPage() - 1) * $announcements->perPage() + 1 }}</strong>–<strong>{{ min($announcements->currentPage() * $announcements->perPage(), $announcements->total()) }}</strong> dari <strong>{{ $announcements->total() }}</strong> pengumuman
            </p>
            <div>
                {{ $announcements->links() }}
            </div>
