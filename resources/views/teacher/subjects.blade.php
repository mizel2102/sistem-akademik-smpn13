...
@section('page-title', 'Mata Pelajaran Saya - Portal Guru')
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
                ...
                <h2 class="text-3xl font-extrabold text-white">
                    {{ $primarySubject?->name ?? 'Matematika' }}
                </h2>
                <p class="mt-2 text-sm text-slate-200">
                    Kode Mapel: <span class="font-mono text-amber-400 font-bold px-2 py-0.5 bg-slate-800/80 rounded">{{ $primarySubject?->code ?? 'MTK' }}</span> | Pengampu: <strong class="text-white">{{ Auth::user()->name }}</strong>
                </p>
            </div>
            <div class="rounded-xl bg-white/10 backdrop-blur-md p-4 border border-white/10 text-center min-w-[160px]">
                ...
                <p class="text-3xl font-extrabold text-amber-400 mt-1">{{ $classes->count() }}</p>
            </div>
        </div>
    </div>
                ...
                <div class="rounded-2xl bg-white p-5 shadow-sm border border-slate-100 hover:shadow-md transition">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-xs font-mono text-slate-500 bg-slate-100 px-2 py-0.5 rounded">
                            Token: {{ $class->access_token ?? '-' }}
                        </span>
                    </div>
                    <h4 class="mt-4 text-xl font-bold text-slate-900">{{ $class->name }}</h4>
                    <p class="mt-1 text-xs text-slate-500">Jadwal: {{ $class->schedule ?? 'Belum Diatur' }}</p>
                    
                    <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-600">
                        <span>Total Siswa: <strong class="text-navy font-bold">{{ $class->students_count ?? $class->students()->count() }}</strong></span>
                        <a href="{{ route('teacher.grades.index', ['class_id' => $class->id]) }}" class="font-semibold text-blue-600 hover:underline">
                            Input Nilai &rarr;
                        </a>
                    </div>
                </div>
