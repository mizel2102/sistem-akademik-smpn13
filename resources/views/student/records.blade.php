...
@section('breadcrumb', 'Siswa › Nilai Saya')
...
<div class="space-y-6">
    ...
    <div class="rounded-2xl bg-white p-6 shadow-sm">
        <form method="GET" class="grid gap-4 md:grid-cols-3 md:items-end">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('student.records.index') }}" class="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">
                    Reset
                </a>
            </div>
        </form>
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div class="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <p class="mt-4 text-3xl font-bold text-navy">{{ number_format($grades->avg('score') ?? 0, 1) }}</p>
        </div>

        <div class="rounded-2xl bg-white p-6 shadow-sm">
            <p class="text-sm font-semibold uppercase tracking-wide text-slate-500">Nilai Tertinggi</p>
            <p class="mt-4 text-3xl font-bold text-navy">{{ $grades->max('score') ?? 0 }}</p>
        </div>

        <div class="rounded-2xl bg-white p-6 shadow-sm">
            <p class="text-sm font-semibold uppercase tracking-wide text-slate-500">Nilai Terendah</p>
            <p class="mt-4 text-3xl font-bold text-navy">{{ $grades->min('score') ?? 0 }}</p>
        </div>
    </div>
            ...
            @foreach($grades->groupBy(fn($grade) => $grade->subject?->name ?? 'Tanpa Mapel') as $subject => $subjectGrades)
                ...
                <div>
                    ...
                    <div class="mt-2 h-3 overflow-hidden rounded-full bg-slate-100">
                        <div class="h-full rounded-full bg-navy" style="width: {{ $width }}%;"></div>
                    </div>
                </div>
            @endforeach
        </div>
