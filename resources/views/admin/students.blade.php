...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
            <a href="{{ route('admin.students.export') }}" class="inline-flex items-center gap-2 rounded-xl bg-green-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-green-700 transition">
                Export Excel
            </a>
            ...
            <a href="{{ route('admin.students.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-navy px-4 py-2 text-sm font-semibold text-white shadow hover:bg-opacity-90 transition">
    ...
    <form method="GET" class="rounded-2xl bg-white p-4 shadow-sm">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-end">
            ...
            @if(request()->hasAny(['search', 'class']))
                <a href="{{ route('admin.students.index') }}" class="rounded-lg border border-slate-200 px-6 py-2.5 font-medium text-slate-700 transition hover:bg-slate-50">
                    Reset
                </a>
            @endif
                        ...
                        <tr class="{{ $i % 2 === 1 ? 'bg-slate-50' : '' }} border-t border-slate-200 transition hover:bg-slate-100">
                            ...
                            <td class="px-6 py-4 text-sm">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-medium text-slate-900">{{ $student->user->name ?? '-' }}</p>
                            ...
                            <td class="px-6 py-4 text-sm">
                                <div class="flex items-center justify-center gap-2">
                                    <!-- Detail Button -->
                                    <a href="{{ route('admin.students.show', $student) }}" class="rounded-lg p-2 text-slate-600 transition hover:bg-slate-100 hover:text-navy" title="Detail">
                                    ...
                                    <!-- View Report -->
                                    <a href="{{ route('admin.reports.rapor', $student) }}" class="rounded-lg p-2 text-slate-600 transition hover:bg-slate-100 hover:text-blue-600" title="Lihat Rapor">
                                        <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    ...
                                    <!-- Download Report PDF -->
                                    <a href="{{ route('admin.reports.rapor.pdf', $student) }}" class="rounded-lg p-2 text-slate-600 transition hover:bg-slate-100 hover:text-green-600" title="Unduh PDF Rapor">
                                        <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    ...
                                    <!-- Edit Button -->
                                    <a href="{{ route('admin.students.edit', $student) }}" class="rounded-lg p-2 text-slate-600 transition hover:bg-slate-100 hover:text-gold" title="Edit">
                                        <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                    ...
                                    >
                                        <div class="rounded-2xl bg-white p-6 shadow-2xl max-w-sm mx-4">
                                            ...
                                            <p class="mb-6 text-center text-sm text-slate-600">Data siswa <strong>{{ $student->user->name ?? 'ini' }}</strong> akan dihapus permanen.</p>
                                            <div class="flex gap-3">
                                                ...
                                                <form method="POST" action="{{ route('admin.students.destroy', $student) }}" class="flex-1">
                                                    @csrf
                                                    @method('DELETE')
    ...
    @if($students && $students->total() > 0)
        <div class="flex items-center justify-between">
            <p class="text-sm text-slate-600">
                Menampilkan <strong>{{ ($students->currentPage() - 1) * $students->perPage() + 1 }}</strong>–<strong>{{ min($students->currentPage() * $students->perPage(), $students->total()) }}</strong> dari <strong>{{ $students->total() }}</strong> siswa
            </p>
            <div>
                {{ $students->links() }}
            </div>
