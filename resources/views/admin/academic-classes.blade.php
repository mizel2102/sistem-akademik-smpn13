...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.academic-classes.create') }}" class="inline-flex items-center gap-2 rounded-xl bg-navy px-6 py-3 font-semibold text-white shadow-lg hover:bg-opacity-90 transition">
        ...
        @forelse($classes ?? [] as $class)
            <div x-data="{ showDeleteModal: false }" class="relative rounded-2xl bg-white p-5 shadow-sm transition hover:shadow-md border border-slate-100 flex flex-col justify-between">
                <div>
                    ...
                    <div class="mb-4 flex items-start justify-between">
                        ...
                        <div class="flex items-center gap-1">
                            <!-- Edit Button -->
                            <a href="{{ route('admin.academic-classes.edit', $class) }}" class="rounded-lg p-2 text-slate-500 transition hover:bg-slate-100 hover:text-navy" title="Edit Kelas">
                                <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            ...
                            >
                                <div @click.outside="showDeleteModal = false" class="rounded-2xl bg-white p-6 shadow-2xl max-w-sm w-full">
                                    ...
                                    <p class="mb-6 text-center text-sm text-slate-600">Kelas <strong>{{ $class->name }}</strong> akan dihapus secara permanen.</p>
                                    <div class="flex gap-3">
                                        ...
                                        <form method="POST" action="{{ route('admin.academic-classes.destroy', $class) }}" class="flex-1">
                                            @csrf
                                            @method('DELETE')
                    ...
                    <!-- Class Name -->
                    <h3 class="mb-2 text-lg font-bold text-navy">{{ $class->name }}</h3>

                    <!-- Room Info -->
                    <p class="mb-1 text-sm text-slate-600">
                        <span class="font-medium text-slate-700">Ruang:</span> {{ $class->room ?? '-' }}
                    </p>

                    <!-- Wali Kelas -->
                    <p class="mb-4 text-sm text-slate-600">
                        <span class="font-medium text-slate-700">Wali Kelas:</span>
                        {{ $class->teacher?->user?->name ?? 'Belum ada wali kelas' }}
                    </p>

                    <div class="mb-4 flex flex-wrap items-center gap-2">
                        <span class="inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">Kapasitas: {{ $class->capacity ?? 0 }}</span>
                        @php
                            $statusMap = [
                        ...
                        @endphp
                        <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold {{ $status['bg'] }} {{ $status['text'] }}">{{ $status['label'] }}</span>
                    </div>
                </div>
                ...
                <div class="border-t border-slate-100 pt-3 flex items-center justify-between mt-2">
                    ...
                    <a href="{{ route('admin.academic-classes.show', $class) }}" class="inline-flex items-center gap-1 text-sm font-semibold text-navy transition hover:text-gold">
                        Detail
